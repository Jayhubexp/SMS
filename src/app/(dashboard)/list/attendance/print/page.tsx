...
export default async function PrintAttendancePage({
	...
	if (classId) {
		...
		// FIX: Query 'students' table directly instead of non-existent 'class_enrollments'
		const { data: studentData, error } = await supabaseAdmin
			...
			.eq("class_id", classId); // Filter by class_id directly on students table
		...
		if (studentData) {
			// Sort explicitly by last name in Javascript since deep sorting in Supabase can be tricky
			students = studentData.sort((a: any, b: any) =>
					...
					<>
						...
						<script
							dangerouslySetInnerHTML={{
								__html: `document.getElementById('printBtn').onclick = () => window.print()`,
							}}
						/>
					</>
				)}
