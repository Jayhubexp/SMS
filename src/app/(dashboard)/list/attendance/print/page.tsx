...
export default async function PrintAttendancePage({
	...
	if (classId) {
		...
		// FIX: Query 'first_name' and 'last_name' directly from the students table
		...
		if (studentData) {
			// FIX: Sort using the direct properties
			...
			{classId ? (
				<div className='print:w-full'>
								...
								students.map((student: any, index: number) => (
									<tr key={student.id} className='break-inside-avoid'>
										...
										<td className='border border-black p-2 font-medium'>
											{/* Now these match the fetched columns */}
					...
					<div className='mt-8 flex justify-between text-xs'>
						<div>
							<p>
								Class Teacher Signature: __________________________
							</p>
						</div>
						<div>
