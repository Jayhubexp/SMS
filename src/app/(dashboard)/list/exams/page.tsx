...
import { examsData }_ from "@/lib/data";
...
const ExamListPage = () => {
  const { role } = useAuth();
  const canModify = ["system_administrator", "teacher"].includes(role);
  ...
  return (
    <div className="bg-white dark:bg-dark-bgSecondary p-4 rounded-md flex-1 m-4 mt-0">
      ...
      <div className="flex items-center justify-between">
        <h1 className="hidden md:block text-lg font-semibold dark:text-dark-text">All Exams</h1>
        <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
          ...
          <div className="flex items-center gap-4 self-end">
            ...
            {canModify && <FormModal table="exam" type="create" />}
          </div>
        </div>
      </div>
      {/* LIST */}
